...
final class PaieController extends AbstractController
{
    ...
    public function generer(Request $request, PaieRepository $paieRepository, PaginatorInterface $paginator): Response
    {
        $entreprise = $this->getUser()->getEntreprise();
        [$mois, $annee] = $this->resolvePeriod($request);

        $allPaies = $paieRepository->findByEntrepriseAndPeriod($entreprise, $mois, $annee);
        $paies = $paginator->paginate(
            $paieRepository->createByEntrepriseAndPeriodQueryBuilder($entreprise, $mois, $annee),
        ...
        return $this->render('rh/paie/generer.html.twig', [
            ...
            'totalNet' => $totalNet,
            'totalPaid' => $totalPaid,
            'totalCount' => count($allPaies),
        ]);
    }

    #[Route('/paie/calculer-tout', name: 'app_rh_paie_calculer_tout', methods: ['POST'])]
    public function calculerTout(
        ...
        return $this->redirectToRoute('app_rh_paie_preparer', ['mois' => $mois, 'annee' => $annee]);
    }
    ...
    #[Route('/paie/{id}/valider', name: 'app_rh_paie_valider', methods: ['POST'])]
    public function valider(Request $request, Paie $paie, EntityManagerInterface $em, PayslipService $payslipService, AvanceSalaireRepository $avanceSalaireRepository): Response
    {
        if (!$this->isCsrfTokenValid('valider-'.$paie->getId(), (string) $request->request->get('_token'))) {
            ...
            return $this->redirectToRoute('app_rh_paie_generer');
        }
        ...
        if ($paie->isPaid()) {
            ...
            return $this->redirectToRoute('app_rh_paie_generer', ['mois' => $mois, 'annee' => $annee]);
        }
    ...
    #[Route('/paie/valider-selection', name: 'app_rh_paie_valider_selection', methods: ['POST'])]
    ...
    #[Route('/paie/{id}/voir', name: 'app_rh_paie_voir', methods: ['GET'])]
    public function voir(Paie $paie): Response
    {
        $this->assertOwnership($paie);

        return $this->render('rh/paie/voir.html.twig', [
            'payslip' => $paie,
            'employee' => $paie->getEmployee(),
    ...
    #[Route('/paie/{id}/telecharger', name: 'app_rh_paie_telecharger', methods: ['GET'])]
    ...
    #[Route('/paie/exporter', name: 'app_rh_paie_exporter', methods: ['GET'])]
